...
class EnslaverSearch:
    ...
    def execute(self, fields):
        ...
        # for the ORM queries we were generating.
        # 
        # Our solution requires the use of a subquery to fetch the unique
        # enslaver ids that match a query and apply any sorting to the results.
        ...
        MultiValueHelper.set_group_concat_limit()
        if is_fuzzy:
            leadq = _auto_select_related_fields(q, fields)
        else:
            q = q.values('id')
            leadq = _auto_select_related_fields(EnslaverIdentity.objects, fields)
        for helper in self.all_helpers:
            leadq = helper.adapt_query(leadq)
        leadq = leadq.values(*fields)
        ...
        class RawQueryWrapper:
            ...
            def __init__(self, q, lenq):
                self.q = q
                self.lenq = lenq

            def __len__(self):
                countq = RawQuery(f"SELECT COUNT(*) FROM ({self.lenq.sql}) XXXX", self.lenq.using, self.lenq.params)
            ...
            def __getitem__(self, k):
                if isinstance(k, slice):
                    sliced = RawQuery(self._replace_limit(f"LIMIT {k.start},{k.stop - k.start}"), self.q.using, self.q.params)
                    return RawQueryWrapper(sliced, self.lenq)
            ...
            def _replace_limit(self, replacement):
                return self.q.sql.replace("LIMIT 0,0", replacement)
        ...
        if is_fuzzy:
            ...
            (lqs, lpp) = leadq.query.sql_with_params()
            (qs, qpp) = q.query.sql_with_params()
            q = EnslaverIdentity.objects.raw(f"{lqs} INNER JOIN ({qs}) AS matches ON matches.id=past_enslaveridentity.id LIMIT 0,0", lpp + qpp)
            q = RawQueryWrapper(q.query, EnslaverIdentity.objects.raw(qs, qpp).query)
        return q
