...
from voyages.apps.past.models import Enslaved,LanguageGroup
...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		for link in links:
			...
			G.add_edge(s_id,t_id,distance=distance,id=e,tags=["oceanic_leg"],curve=True)
			e+=1
...
# 		enslaved_individuals.select_related(
# 			'language_group',
...
# 		)
		...
		individuals_with_languagegroup=enslaved_individuals.filter(language_group__isnull=False)
...
# 				"connect_to_tags":[("final_destination","source","all"),("offramp","target","single")]
# 			},
# 			{
				...
				for connect_to_tag in connect_to_tags:
					...
					if mode=="single":
						...
						if curve==False and distance > threshold_for_straight:
							curve=True
						if as_type=="source":
							G.add_edge(id,closest_neighbor,distance=distance,id=e,curve=curve)
						else:
							G.add_edge(closest_neighbor,id,distance=distance,id=e,curve=curve)
						e+=1
					else:
						...
						for comp_node in comp_nodes:
							...
							if curve==False and distance > threshold_for_straight:
								...
								this_curve=curve
							if as_type=="source":
								G.add_edge(id,comp_node,id=e,distance=distance,curve=this_curve)
							else:
								G.add_edge(comp_node,id,id=e,distance=distance,curve=this_curve)
							e+=1
...
# 				print(a,b)
		...
		d=open(base_path+'routes_curves.py','w')
		d.write("route_curves="+str(routes))
		d.close()
		...
		for n in G.nodes:
			...
			if 'oceanic_waypoint' not in node['tags']:
				...
				name=node['name']
				pk=node['pk']
				geo_points[n]=(coords,name,pk)
		
		d=open(base_path+'routes_points.json','w')
