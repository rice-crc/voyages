...
class Command(BaseCommand):
    ...
    def handle(self, sources_xls, *args, **options):
        ...
        while idx < sheet.nrows and sheet.cell_value(idx, full_ref_col) and not sheet.cell_value(idx, full_ref_col) == '':
            short_ref = sheet.cell_value(idx, short_ref_col)
            full_ref = sheet.cell_value(idx, full_ref_col)
            src_type = models.VoyageSourcesType.objects.get(group_id=sheet.cell_value(idx, src_type_col))
