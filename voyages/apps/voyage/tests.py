...
class SearchTest(TestCase):
    ...
    def test_search_geographical(self, perform_search_func):
        ...
        expected = [60412, 60803, 60807, 60820, 60833, 60834, 60999, 50203, 50299]
        self.assertEqual(expected, qdict['var_imp_port_voyage_begin_idnum__in'])
    @patch('voyages.apps.voyage.views.perform_search')
    ...
    def test_search_select(self, perform_search_func):
        self.client.get('/voyage/search?used_variable_names=var_outcome_slaves&var_outcome_slaves_choice_field=1%3B2%3B3%3B5%3B6&time_span_from_year=1514&time_span_to_year=1866')
        self.assertEqual(len(perform_search_func.call_args_list), 1)
        args = perform_search_func.call_args_list[0][0]
        qdict = args[0]
