...
def get_set_from_prefix(row, prefix):
    ...
    return {
        o for o in row_set
        if o is not None and o.strip() != '' and o.strip() != "."
    }
...
def compare_csv(r1,
    ...
    for (a, b) in pairs:
        ...
        for k in keys:
            ...
            if dx and dy and int(dx.group(1)) == int(dy.group(1)) and int(
                    dx.group(2)) == int(dy.group(2)) and int(
                        dx.group(3)) == int(dy.group(3)):
                continue
            try:
        ...
        for prefix in ["owner", "captain"]:
            prefix_diffs = [
                key for key in list(diffs.keys())
            ...
            if prefix_set_a != prefix_set_b:
                diffs[prefix + "_set"] = "Set of " + prefix + " is different: " + \
                    str(prefix_set_a) + " vs. " + str(prefix_set_b)
        if len(diffs) > 0:
            bad.append((a, b, diffs))
    print(f"Row count {count}. Issues {len(bad)}. Warnings: {len(warning)}.")
    return (bad, warning)
...
def check_files(name,
    ...
    return compare_csv(imported_csv_data, exported_csv_data, default_values,
                       ignore_keys, key_map)


def check_intra(imported_csv_file, exported_csv_file):
    return check_files(
        "intra", imported_csv_file, exported_csv_file, {"evgreen": "false"},
        default_ignore_keys | set([
            "afrinfo", "othercargo", "dateland1", "dateland2", "datedep",
            "datebuy", "dateleftafr", "voyageid2"
        ]), {"datebuy1": "datebuy"})


def check_tast(imported_csv_file, exported_csv_file):
    return check_files(
        "intra", imported_csv_file, exported_csv_file, {},
        default_ignore_keys | set([
            "datebuy", "datedep", "datedepam", "dateland1", "dateland2",
            "dateland3", "dateend"
        ]))
    ...
    if len(delta[0]) > 0:
        ...
        for item in delta[0]:
            ...
            if item[1]["voyageid"] != voyage_id:
                print("- Voyage id mismatch "
                      f"{voyage_id}/{item[0]['voyageid']}")
            else:
